...
//<== Extended Failed/Success Statistic by NetF [shadow2004]
                if(thePrefs.GetLogUlDlEvents())
                    AddDebugLogLine(DLP_VERYLOW, false, _T("Download session ended: NNP. You don't need any parts from this client. User: %s in SetDownloadState(). New State: %i, Length: %s, Transferred: %s."), DbgGetClientInfo(), nNewState, CastSecondsToHM(GetDownTimeDifference(false)/1000), CastItoXBytes(GetSessionDown(), false, false));
