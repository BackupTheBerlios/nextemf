...
bool CPartFile::GetNextEmptyBlockInPart(uint16 partNumber, Requested_Block_Struct *result, uint32 bytesRequested) /*const*/
{
	...
	CList<Requested_Block_Struct> availableBlocks;
		...
		if(currentGap->start <= partEnd && currentGap->end >= partStart)
		{
			for(uint32 i = 0; i < 53; ++i)
			{
				...
				if ((currentGap->start <= blockEnd) && (currentGap->end >= blockStart))
				{
					...
					Requested_Block_Struct newBlock;
//					struct dataBlock newBlock;
					newBlock.StartOffset = start;
					newBlock.EndOffset = end;
	...
	for (POSITION i =  requestedblocks_list.GetHeadPosition();i != 0; )
	{
		...
		{
			const Requested_Block_Struct avail_block = availableBlocks.GetAt(j);
//			const dataBlock avail_block = availableBlocks.GetAt(j);
			if (avail_block.StartOffset <= cur_block->EndOffset && avail_block.EndOffset >=cur_block->StartOffset)
//			if (avail_block.start <= cur_block->EndOffset && avail_block.end >=cur_block->StartOffset)
			{
				availableBlocks.RemoveAt(j);
				Requested_Block_Struct newBlock;
//				struct dataBlock newBlock;
				if (avail_block.StartOffset < cur_block->StartOffset)
				{
				newBlock.StartOffset = avail_block.StartOffset;
				newBlock.EndOffset = cur_block->StartOffset - 1;
				availableBlocks.AddTail(newBlock);
				}
				if (avail_block.EndOffset > cur_block->EndOffset)
				{
				newBlock.StartOffset = cur_block->EndOffset + 1;
				newBlock.EndOffset = avail_block.EndOffset;
				availableBlocks.AddTail(newBlock);
				}
/*				if (avail_block.start < cur_block->StartOffset)
	...
	const Requested_Block_Struct avail_block = availableBlocks.GetAt(idx);
	start = avail_block.StartOffset;
	end = avail_block.EndOffset;
			...
			if(sender->m_lastPartAsked == 0xffff){
				...
				if(chunksList.IsEmpty() == TRUE){
					...
					for(uint16 i=0; i < partCount; i++){
						if(sender->IsPartAvailable(i) == true && GetNextEmptyBlockInPart(i, NULL,NULL) == true){
